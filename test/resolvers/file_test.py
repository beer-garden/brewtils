...
def test_download(monkeypatch, resolver, target_file_id, target_file_content):
    writer = Mock()
    monkeypatch.setattr(
        resolver.client, "download_chunked_file", Mock(return_value=target_file_content)
    )
...
def test_upload_filename_does_not_exist(monkeypatch, resolver):
    monkeypatch.setattr(
        resolver.client, "upload_chunked_file", Mock(side_effect=ValidationError())
    )
    with pytest.raises(ValidationError):
...
def test_upload_filename(monkeypatch, resolver, example_file, target_file_id):
    monkeypatch.setattr(
        resolver.client, "upload_chunked_file", Mock(return_value=target_file_id)
    )
    ret = resolver.upload(example_file)
    assert resolver.client.upload_chunked_file.call_count == 1
...
def test_upload_file_descriptor(resolver, example_file):
    ...
    fd.close()
    assert resolver.client.upload_chunked_file.call_count == 1
    resolver.client.upload_chunked_file.assert_called_with(fd)
