...
def _update_func_command(func_command, generated_command):
    """Updates a Command's attributes

    Will replace all attributes EXCEPT parameters on func_command with values from
    generated_command.

    Args:
        func_command: The Command to be updated
        generated_command: Command with values to use
    ...
    for attr in [
        ...
        setattr(func_command, attr, getattr(generated_command, attr))


def _generate_command_from_function(func):
    """Generate a Command from a function.

    The generated Command will have the following attributes:
        * Command name will be the function name
        * Command description will be the first line of the function docstring.
        * Command Parameters will be generated by _generate_params_from_function
    ...
    # TODO - This currently prefers Python 2. Switch the order for version 3.
    command_name = getattr(func, "func_name", func.__name__)
    docstring = getattr(func, "func_doc", func.__doc__)
