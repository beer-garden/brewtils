...
# The wrapt module has a cool feature where you can disable wrapping a decorated function, instead just using the
# original function. This is pretty much exactly what we want - we aren't using decorators for their 'real' purpose of
# wrapping a function, we just want to add some metadata to the function object. So we'll disable the wrapping normally,
# but we need to test that enabling the wrapping would work.
...
def command(_wrapped=None, command_type='ACTION', output_type='STRING', schema=None, form=None, template=None,
    ...
    :param _wrapped: The function to decorate. This is handled as a positional argument and shouldn't be explicitly set.
    ...
    :param icon_name: The icon name. Should be either a FontAwesome or a Glyphicon name (e.g. fa-heart).
    :return: The decorated function.
    """
    if _wrapped is None:
        return functools.partial(command, command_type=command_type, output_type=output_type, schema=schema, form=form,
                                 template=template, icon_name=icon_name)
    ...
    resolved_mod = _resolve_display_modifiers(_wrapped, generated_command.name, schema=schema, form=form,
                                              template=template)
...
def parameter(_wrapped=None, key=None, type=None, multi=None, display_name=None, optional=None, default=None,
    ...
    @parameter(key="message", description="Message to echo", optional=True, type="String", default="Hello, World!")
    def echo(self, message):
        return message

    :param _wrapped: The function to decorate. This is handled as a positional argument and shouldn't be explicitly set.
    :param key: String specifying the parameter identifier. Must match an argument name of the decorated function.
    :param type: String indicating the type to use for this parameter.
    :param multi: Boolean indicating if this parameter is a multi. See documentation for discussion of what this means.
    ...
    :param default: The value this parameter will be assigned if not overridden when creating a request.
    :param description: An additional string that will be displayed in the user interface.
    :param choices: List or dictionary specifying allowed values. See documentation for more information.
    ...
    :param is_kwarg: Boolean indicating if this parameter is meant to be part of the decorated function's kwargs.
    :param model: Class to be used as a model for this parameter. Must be a Python type object, not an instance.
    :param form_input_type: Only used for string fields. Changes the form input field (e.g. textarea, email, password)
    :return: The decorated function.
    """
    if _wrapped is None:
        return functools.partial(parameter, key=key, type=type, multi=multi, display_name=display_name,
                                 optional=optional, default=default, description=description, choices=choices,
                                 nullable=nullable, maximum=maximum, minimum=minimum, regex=regex, is_kwarg=is_kwarg,
    ...
    if key is None:
        raise PluginParamError("Found a parameter definition without a key for command '%s'" % cmd.name)

    # If the command doesn't already have a parameter with this key then the method doesn't have an explicit keyword
    # argument with <key> as the name. That's only OK if this parameter is meant to be part of the **kwargs.
    ...
    if param is None:
        if is_kwarg:
            ...
            raise PluginParamError(("Parameter '%s' was not an explicit keyword argument for command '%s' and was " +
                                   "not marked as part of kwargs (should is_kwarg be True?)") % (key, cmd.name))
    ...
    if model is not None:
        ...
        # If the model is not nullable and does not have a default defined we will try to generate a default using
...
def _generate_params_from_function(func):
    """Generate Parameters from function arguments. Will set the Parameter key, default value, and optional value."""
...
def _generate_nested_params(model_class):
    ...
    for parameter_definition in model_class.parameters:
        ...
        parameters_to_return.append(Parameter(key=key, type=parameter_type, multi=multi, display_name=display_name,
                                              optional=optional, default=default, description=description,
                                              choices=choices, parameters=nested_parameters, nullable=nullable,
                                              maximum=maximum, minimum=minimum, regex=regex))
    return parameters_to_return
        ...
        if isinstance(value, six.string_types):
            try:
                if value.startswith('http'):
                    ...
                    raise PluginParamError("%s specified for command '%s' was not a definition, file path, or URL" %
                                           (key, command_name))
            except Exception as ex:
                raise PluginParamError("Error reading %s definition from '%s' for command '%s': %s" %
                                       (key, value, command_name, ex))
...
def _format_choices(choices):
    ...
    if not isinstance(choices, (list, six.string_types, dict)):
        ...
        if not choices.get('value'):
            raise PluginParamError("No 'value' provided for choices. You must at least provide valid values.")
        ...
        if choice_type is None:
            ...
            if (choice_type == 'command' and not isinstance(value, (six.string_types, dict))) \
                ...
                raise PluginParamError("Invalid choices value type '%s' - Valid value types for choice type '%s' are %s"
        ...
        if choice_type == 'command':
            ...
            if isinstance(value, dict):
                ...
                if unparsed_value is None:
                    raise PluginParamError('Specifying a static choices dictionary requires a "key_reference" field '
                                           'with a reference to another parameter ("key_reference": "${param_key}")')
