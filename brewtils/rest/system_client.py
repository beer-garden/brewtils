...
class SystemClient(object):
    ...
    SystemClient creation:
        ...
        Pass additional keyword arguments for more granularity:
            ...
            version_constraint:
                Allows specifying a particular system version. Can be a version literal ('1.0.0') or the special value
                'latest.' Using 'latest' will allow the the SystemClient to retry a request if it fails due to a missing
                system (see Creating Requests).

            default_instance:
                The instance name to use when creating a request if no other instance name is specified. Since each
                request must be addressed to a specific instance this is a convenience to prevent needing to specify the
                'default' instance for each request.

            always_update:
                Always attempt to reload the system definition before making a request. This is useful to ensure
                Requests are always made against the latest version of the system. If not set the System definition will
                be loaded once (upon making the first request) and then only reloaded if a Request fails.
    ...
    Making a Request:
        ...
        It is also possible to create the SystemClient in non-blocking mode by specifying blocking=False. In this case
        the request creation will immediately return a Future and will spawn a separate thread to poll for Request
        completion. The max_concurrent parameter is used to control the maximum threads available for polling.
        ...
        If the request creation process fails (e.g. the command failed validation) and version_constraint is 'latest'
        then the SystemClient will check to see if a different version is available, and if so it will attempt to make
    ...
    def __init__(self, host, port, system_name, version_constraint='latest', default_instance='default',
                 always_update=False, timeout=None, max_delay=30, api_version=None, ssl_enabled=False, ca_cert=None,
                 blocking=True, max_concurrent=None, client_cert=None, url_prefix=None, ca_verify=True):
        ...
        self._easy_client = EasyClient(host, port, ssl_enabled=ssl_enabled, api_version=api_version, ca_cert=ca_cert,
    ...
    def create_bg_request(self, command_name, **kwargs):
        ...
        Example:
            ...
            requests.append(client.create_bg_request('command_1'))               # No arguments
            requests.append(client.create_bg_request('command_2', arg_1='Hi!'))  # arg_1 will be passed as a parameter
        ...
        if command_name in self._commands:
            return partial(self.send_bg_request, _command=command_name, _system_name=self._system.name,
                           _system_version=self._system.version, _system_display=self._system.display_name,
                           _output_type=self._commands[command_name].output_type, _instance_name=self._default_instance,
                           **kwargs)
        else:
    ...
    def send_bg_request(self, **kwargs):
        ...
        # If the request fails validation and the version constraint allows, check for a new version and retry
        try:
    ...
    def load_bg_system(self):
        ...
        if self._version_constraint == 'latest':
            ...
            self._system = sorted(systems, key=lambda x: x.version, reverse=True)[0] if systems else None
        else:
    ...
    def _wait_for_request(self, request):
        ...
        while request.status not in Request.COMPLETED_STATUSES:
            ...
            if self._timeout and total_wait_time > self._timeout:
                raise BrewmasterTimeoutError("Timeout reached waiting for request '%s' to complete" % str(request))
    ...
    def _construct_bg_request(self, **kwargs):
        ...
        return Request(command=command, system=system_name, system_version=system_version, instance_name=instance_name,
                       comment=comment, output_type=output_type, parent=parent, metadata=metadata, parameters=kwargs)


class BrewmasterSystemClient(SystemClient):
    def __init__(self, *args, **kwargs):
        warnings.warn("Call made to 'BrewmasterSystemClient'. This name will be removed in version 3.0, please use "
                      "'SystemClient' instead.", DeprecationWarning, stacklevel=2)
